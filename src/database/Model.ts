...
export default class Model extends (sequelize.Model as any) {
  ...
  static async init(): Promise<any> {
    ...
    // Acquire shared Sequelize instance from the singleton manager
    this.db = await DatabaseManager.getInstance();

    // Register model schema with Sequelize
    // freezeTableName: use exact table name (no pluralization)
    // timestamps: disable automatic createdAt/updatedAt columns
  ...
  static association(): void {
    // Override in subclass to define hasOne, hasMany, belongsTo, etc.
  }
